...
// dd($invoices);
$driver_commision = $user->driver->amount;
$driver_credits = $user->driver->credit;
...
@foreach ($invoices as $invoice)
 ...
 @php
    ...
    $earn_by_driver = $invoice->grand_price;

    if($user->driver->supplier){
        $supplier_commision = $user->driver->supplier->commission;
    }else{
        $supplier_commision = 0;
    }

    $supplier_profit = $earn_by_driver * ($supplier_commision / 100);

  

    if( $supplier_profit <= 0){
        // Admin's Driver
        
        $driver_profit = $earn_by_driver  * ($driver_commision / 100);
        $admin_profit = $earn_by_driver -  $driver_profit;
    }else{
        // Supplier Driver
        $driver_profit = $supplier_profit  * ($driver_commision / 100);
        $admin_profit = $earn_by_driver -  $supplier_profit;
    }

     
   
    $total_driver += $driver_profit;

    $total_profit += $earn_by_driver;
 ...
 <tr>
    ...
    <td>
        {{Config('currency-symbol')}} {{ $earn_by_driver }}
    </td>
    ...
    <td>
        {{Config('currency-symbol')}} {{ $driver_profit }}
    </td>
