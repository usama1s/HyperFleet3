...
use App\Models\Booking_tr;
use App\Models\Supplier;
use App\Models\User;



$trs = DB::table('bookings')
            ->join('booking_invoices', 'bookings.id', '=', 'booking_invoices.booking_id')
            ->join('booking_balance_sheets', 'bookings.id', '=', 'booking_balance_sheets.booking_id')
            ->where("booking_invoices.status","paid")
            ->where("bookings.driver_id",$user->id)
            ->select('booking_balance_sheets.*','booking_invoices.*','bookings.grand_price')->get();
...
@foreach ($trs as $tr)
 ...
 @php
    ...
    $total_booking_amount += $tr->grand_price;
 ...
 <tr>
    ...
    <td>{{$sno++}}</td>
    <td>
        <a href="{{ route('booking-invoice.show',$tr->id)}}">
            tr#{{ $tr->id }}
        </a>
    </td>
     
    <td>{{ $tr->status }}</td>
    <td>{{  Booking::getPaymentMethod($tr->booking_id) }}</td>
    <td>
        {{Config('currency-symbol')}} {{ $tr->grand_price }}
    </td>

    <td>
        {{Config('currency-symbol')}} {{ $tr->credit }}
    </td>
    
    <td> {{Config('currency-symbol')}} {{ $tr->debit }}</td>

   
    <td> {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $tr->updated_at)->format('Y-m-d') }}</td>
