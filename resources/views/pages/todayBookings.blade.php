     ...
     <div class="card card-primary card-outline card-outline-tabs" style="border-color:transparent">
        <div class="card-header p-0 border-bottom-0">
           <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
              <li class="nav-item">
                 <a class="nav-link active" id="custom-tabs-three-home-tab" data-toggle="pill" href="#custom-tabs-three-home" role="tab" aria-controls="custom-tabs-three-home" aria-selected="false">Today's Bookings</a>
              </li>

              <li class="nav-item">
                 <a class="nav-link" id="custom-tabs-three-profile-tab" data-toggle="pill" href="#custom-tabs-three-profile" role="tab" aria-controls="custom-tabs-three-profile" aria-selected="true">Tomorrow's Bookings</a>
              </li>

              <li class="nav-item">
               <a class="nav-link" id="custom-tabs-three-pricing-tab" data-toggle="pill" href="#custom-tabs-three-pricing" role="tab" aria-controls="custom-tabs-three-pricing" aria-selected="true">Unassigned Bookings</a>
            </li>
            
              <li class="nav-item">
               <a class="nav-link" id="custom-tabs-three-bookings-tab" data-toggle="pill" href="#custom-tabs-three-bookings" role="tab" aria-controls="custom-tabs-three-bookings" aria-selected="true">All Bookings</a>
            </li>
           </ul>
        </div>
        <div class="card-body">
...
<table id="manage_bookings" style="width:100% background:#fff; display:none;" class="table table-hover display responsive">
    ...
    <tbody>
        ...
        @php
if($booking->pickup_date == Carbon::today()->format('Y-m-d')){
            ...
            <td>
             ...
             <!--            <img src="{{ asset('public/assets/vehicles') }}/{{$v['image']}}" style="width:100%">   -->
                ...
                <td class="driver_td">        
                    ...
                    <span class="text-info supplier_preview" supplier_id="{{ $booking->supplier_id }}">{{$s['company_name']}}</span>
                ...
                @endphp
                    ...
                    <select class="select_status {{ $selected }} booking_status" id="booking_status_{{$booking->id}}">

                        @foreach ($options as $key =>$value)
                            @if ($key == $booking->status)
                                <option class="text-dark" value="{{$key}}" selected {{$value[2]}}>{{$value[1]}}</option>
                            @else
                                <option class="text-dark" value="{{$key}}" {{$value[2]}}>{{$value[1]}}</option>
                            @endif
...
@include('pages.bookings.bookings-list')
...
@include('pages.bookings.bookings-tbd')
...
@include('pages.bookings.bookings-all')
    ...
    $(document).ready(function() {
        ...
        $(".booking_status").on("change",function(e){
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {id:id ,status:value, loginUser:loginUser},
                success: function(data){
                    ...
                    if(value == 'ready' || value == 'accepted' || value == 'active' || value =='client'){
    ...
    var currency = "{{Config('currency-symbol')}}";
        ...
        Echo.private('notifyChannel')
            .listen('BookingStatusEvent', (event) => {
                ...
                console.log(event);  
                var supplier = null , vehicle =null, driver =null;
