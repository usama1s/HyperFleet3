...
{{-- <ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item"><a href="{{url('pricings')}}">Pricing Schemes </a></li>
...
<div class="page_sub_menu">
    ...
    @can('pricing-create')
        <a class="btn btn-sm btn-success" href="{{ route('pricings.create')}}">Add New</a>
    @endcan
</div>
@endsection
...
<table id="manage_pricings" style="width:100%;" class="table table-hover responsive">
    ...
    <tbody>
        ...
        <td><input type="checkbox" class="multi-select-ids" name="seleted_id[]" value="{{ $list->id }}"></td>
            <td>{{ $sno++}}</td>
            <td>{{$list->title}}</td>
            <td>{{$list->supplier->company_name}}{{$list->bookingagent->company_name}}
            @php
             if($list->supplier->company_name == null && $list->bookingagent->company_name == null) echo "HyperBooking";
             @endphp</td>
            <td>{{$list->base_point}}</td>
