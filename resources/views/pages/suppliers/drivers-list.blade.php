        ...
        @foreach ($drives as $driver)
            ...
            <td width="10px" >{{$sno++}}</td>
                ...
                <td>                            
                    ...
                    @if (auth()->user()->role == 1)
                    <form action="{{ route('drivers.approved_and_disapproved',['id'=> $driver->id]) }}" method="get">
                        <button  class="btn {!!  $driver->admin_approve ? 'btn-success' : 'btn-danger' !!} btn-sm">
                            {!!  $driver->admin_approve ? "Approved" : "Pending" !!}
                        </button>
                    </form>
                    @endif
                    ...
                    @if (auth()->user()->role == 3)
                        <button disabled  class="btn {!!  $driver->admin_approve ? 'btn-success' : 'btn-danger' !!} btn-sm">
                            {!!  $driver->admin_approve ? "Approved" : "Pending" !!}
                        </button>
                    @endif
